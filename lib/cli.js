...
internals.parseCommandLine = function (options) {
    ...
    var validation = internals.validateOptions(result);
...
exports.broadcast = function (log, url) {
...
exports.run = function (options) {
    ...
    if (Array.isArray(options)) {
        config = internals.parseCommandLine(options);
    }
    else {
        var validation = internals.validateOptions(options);
        if (validation.error) {
            throw new Error(validation.error.validation.error.annotate());
        }
    ...
    internals.lastIndexPath = Path.join(Path.dirname(config.path), '.lastindex');
    ...
    var processLog = function (value, callback) {
        ...
        getStats(config.path, function (error, result) {
            ...
            log.get(config.path, value.start, function (bytesRead, logResult) {
                ...
                exports.broadcast(logResult, config.url);

                    if (config.useLastIndex) {
                        return internals.logLastIndex(value.start, wait);
                    }
                    else {
                        wait();
                    }
